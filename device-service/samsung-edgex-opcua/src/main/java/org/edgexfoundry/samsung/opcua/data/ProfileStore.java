...
package org.edgexfoundry.samsung.opcua.data;
...
public class ProfileStore {
    ...
    public void addDevice(Device device) {
        ...
        if (device.getProfile().getDeviceResources() == null) {
            DeviceProfile profile = deviceProfileClient
                    .deviceProfileForName(device.getProfile().getName());
        ...
        for (DeviceObject object : device.getProfile().getDeviceResources()) {
            ...
            /*
             * Add automatically create ValueDescriptor using ObjectDeviceName
             * Because Use ObjectDeviceName as a ValueDescriptor
             */
            ValueDescriptor descriptor = descriptors.stream()
                    .filter(d -> d.getName().equals(object.getName())).findAny().orElse(null);

            // TODO fix coredata bug
            // if (descriptor == null) {
        ...
        /*
         * Delete create ValueDescriptor using operation.parameter Because Not
         * use operation.parameter as a ValueDescriptor
         */
    ...
    private ValueDescriptor createDescriptor(String name, DeviceObject object, Device device) {
        ...
        ValueDescriptor descriptor = new ValueDescriptor(name, value.getMinimum(),
                value.getMaximum(), IoTType.valueOf(value.getType().substring(0, 1)),
                units.getDefaultValue(), value.getDefaultValue(), "%s", null,
