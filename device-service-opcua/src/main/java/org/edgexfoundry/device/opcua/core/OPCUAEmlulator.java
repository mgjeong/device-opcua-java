...
public class OPCUAEmlulator {
	private final static EdgeXLogger logger = EdgeXLoggerFactory.getEdgeXLogger(OPCUAEmlulator.class);
	private EdgeEmulator opcUaEmulator;
	private static OPCUAEmlulator singleton = null;
	...
	private OPCUAEmlulator(DeviceEnroller deviceEnroller, ObjectStore objectCache) {
		opcUaEmulator = new EdgeEmulator();
		this.deviceEnroller = deviceEnroller;
		this.objectCache = objectCache;
	}

	public static OPCUAEmlulator getInstance(DeviceEnroller deviceEnroller, ObjectStore objectCache) {

		if (singleton == null) {
			singleton = new OPCUAEmlulator(deviceEnroller, objectCache);
		}
