...
public class OPCUAMessageHandler {
	 ...
	 * @param [in]
	 *            element element object of json format
	 * @param [in]
	 *            operation opcua command
	 * @param [in]
	 *            providerKey provider key which node
	 * @param [in]
	 *            addr addressable object
	 * @param [in]
	...
	/**
	 * @fn @EdgeMessage getStartMessage(@EdgeElement element, String providerKey,
	 *     Addressable addr, CompletableFuture<@EdgeMessage> future)
	...
	private EdgeMessage getStartMessage(EdgeElement element, String providerKey, Addressable addr,
		...
		String app_name = EdgeJsonFormatter.getStringValueByName(element.getEdgeAttributeList(),
				OPCUAMessageKeyIdentifier.APPLICATION_NAME.getValue());
		String app_uri = EdgeJsonFormatter.getStringValueByName(element.getEdgeAttributeList(),
				OPCUAMessageKeyIdentifier.APPLICATION_URI.getValue());

		EdgeEndpointConfig endpointConfig = new EdgeEndpointConfig.Builder().setApplicationName(app_name)
				.setApplicationUri(app_uri).build();
	...
	/**
	 * @fn @EdgeMessage getStoptMessage(@EdgeElement element, String providerKey,
	 *     Addressable addr, CompletableFuture<@EdgeMessage> future)
	...
	/**
	 * @fn @EdgeMessage getReadMessage(@EdgeElement element, String providerKey,
	 *     Addressable addr, CompletableFuture<@EdgeMessage> future)
	...
	/**
	 * @fn @EdgeMessage getWriteMessage(@EdgeElement element, String providerKey,
	 *     Addressable addr, CompletableFuture<@EdgeMessage> future)
	...
	private EdgeMessage getWriteMessage(EdgeElement element, String providerKey, Addressable addr,
			CompletableFuture<EdgeMessage> future) throws Exception {
		Object inputArgument = EdgeJsonFormatter.getObjectValueByName(element.getEdgeAttributeList(),
				OPCUAMessageKeyIdentifier.INPUT_ARGUMENT.getValue());
		EdgeNodeInfo nodeInfo = new EdgeNodeInfo.Builder().setValueAlias(providerKey).build();
		EdgeVersatility param = new EdgeVersatility.Builder(inputArgument).build();
	...
	/**
	 * @fn @EdgeMessage getSubMessage(@EdgeElement element, String providerKey,
	 *     Addressable addr, CompletableFuture<@EdgeMessage> future)
	...
	private EdgeMessage getSubMessage(EdgeElement element, String providerKey, Addressable addr,
			CompletableFuture<EdgeMessage> future) throws Exception {
		Double sampling_interval = EdgeJsonFormatter.getDoubleValueByName(element.getEdgeAttributeList(),
		...
		EdgeSubRequest sub = new EdgeSubRequest.Builder(EdgeNodeIdentifier.Edge_Create_Sub)
				.setSamplingInterval(sampling_interval).build();
	...
	/**
	 * @fn @EdgeMessage getMethodMessage(@EdgeElement element, String providerKey,
	 *     Addressable addr, CompletableFuture<@EdgeMessage> future)
	...
	private EdgeMessage getMethodMessage(EdgeElement element, String providerKey, Addressable addr,
		...
		if (inputArgument != null) {
			msg = new EdgeMessage.Builder(epInfo).setCommand(EdgeCommandType.CMD_METHOD).setRequest(
					new EdgeRequest.Builder(ep).setMessage(new EdgeVersatility.Builder(inputArgument).build()).build())
					.build();
		} else {
			msg = new EdgeMessage.Builder(epInfo).setCommand(EdgeCommandType.CMD_METHOD)
	...
	/**
	 * @fn @EdgeMessage getEndpointMessage(@EdgeElement element, String providerKey,
	 *     Addressable addr, CompletableFuture<@EdgeMessage> future)
	...
	private EdgeMessage getEndpointMessage(EdgeElement element, String providerKey, Addressable addr,
			CompletableFuture<EdgeMessage> future) throws Exception {
		EdgeEndpointInfo ep = new EdgeEndpointInfo.Builder(getEndpointUrifromAddressable(addr)).setFuture(future)
				.build();
		EdgeNodeInfo nodeInfo = new EdgeNodeInfo.Builder().build();
	 ...
	 * @param [in]
	 *            addressable Addressable object
	 * @return endpoint URI
	 */
