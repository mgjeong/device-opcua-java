...
public class OPCUAMessageHandler {
  ...
  private EdgeMessage getWriteMessage(EdgeElement element, String providerKey, Addressable addr,
      CompletableFuture<EdgeMessage> future) throws Exception {
    Object inputValue = getEdgeAttributeVlaueByName(element.getEdgeAttributeList(),
        DataDefaultValue.CMD_INPUTVALUE.getValue());
    EdgeNodeInfo nodeInfo = new EdgeNodeInfo.Builder().setValueAlias(providerKey).build();
  ...
  private Object getEdgeAttributeVlaueByName(List<EdgeAttribute> edgeAttributeList,
    ...
    for (EdgeAttribute edgeAttr : edgeAttributeList) {
      if (edgeAttr.getName().equals(edgeAttributeName) == true) {
        ...
        return getEdgeAttributeVlaueByName((List<EdgeAttribute>) edgeAttr.getValue(),
            edgeAttributeName);
      }
    }
    return null;
